...
**Spawn both agents in a single tool-call block.** The numbered list above describes responsibilities, not execution order: two `Agent` calls in *one* message run in parallel, two messages run sequentially and waste the wall-clock of whichever pass finishes first waiting on the other. A real failure mode on a prior release: the code agent ran for ~20 minutes alone, then the docs agent for another ~12 minutes, when both could have finished in ~20 minutes total. Issue the calls together.
...
**Tell the subagent explicitly that pushing a fix is not an exit condition.** The agent's turn ends only when every monitored workflow on the latest `main` HEAD has `conclusion: success` (or `skipped` for benign reasons), or when it hits a real blocker it cannot resolve. A failure mode this guards against: the agent fixes one lint violation, pushes it, sees the push succeeded, and returns its summary while the slow jobs are still running. State the loop condition in the prompt verbatim ("re-poll after each push; do not return after a push without re-polling") so it cannot be misread as a "report after first fix" instruction. The parent skill then has to re-spawn babysit to monitor the rest of CI, doubling the wall-clock.
