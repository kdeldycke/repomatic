...
The Python and Click runners hold **independent per-document namespaces**, so a `{python:source}` import isn't visible to a `{click:run}` block and vice versa. The Click asymmetry (see § Live-rendering over captured output) does not carry over: `PythonRunner.execute_source` and `PythonRunner.run_python` both use the two-argument `exec(code, globals)` shape, so `{python:run}` and the `render` family persist their top-level assignments just like `{python:source}`. Seed shared imports in a leading `{python:source}` block all the same (`:hide-source:` if you don't want the snippet rendered), so moving or deleting one example never breaks the ones after it.
...
#### Keeping a committed copy: the `:mirror:` flag

`{python:render}` (the plain form only, not `-myst`/`-rst`) accepts a `:mirror:` flag that keeps a copy of the generated Markdown committed in the source `.md`, between `<!-- mirror -->` / `<!-- mirror-end -->` markers directly below the fence. In mirror mode the directive itself renders nothing (`_run_mirror` returns `[]` unless `:show-source:`); the committed region is what the host parser renders, refreshed in memory at `source-read` time on every build and on disk by `click-extra refresh-directives` (the fourth `repomatic update-docs` phase). This gives you build-time freshness *and* a reviewable, GitHub-rendered copy — the true replacement for the `docs_update.py` marker-region pattern. Mirror generators must `print` `mdformat`-canonical Markdown (`render_table(..., GITHUB)`) or the generator and `mdformat` fight over the region.

**Hard prerequisite — the generator must be importable from the installed package.** The mirror refresher and the `source-read` rebuild both `exec` each block with a bare namespace and no `sys.path` setup, so a mirror block may only import from the installed distribution (`from {package}.x import ...`), never a docs-local helper under `docs/`. A block that does `from docs_update import ...` raises `ModuleNotFoundError` under `click-extra refresh-directives` run from the repo root (verified in extra-platforms), which fails the whole Phase-4 refresh.

If the generators are docs-only tooling you don't want in the public API, ship them in a **private** package module — a leading-underscore name keeps them out of the public surface while still installed and importable. Extra-platforms moved its table/diagram generators from `docs/docs_update.py` into `extra_platforms/_docs.py`; the `:mirror:` blocks do `from extra_platforms._docs import generate_trait_table`. Two things to get right when a docs-only dep is involved: keep the package's runtime clean by never importing the private module from `__init__` (extra_platforms stays dependency-free at import; `_docs` pulls `click_extra.table` only when a docs build calls it), and add a mypy override (`[[tool.mypy.overrides]] module = "click_extra.*" ignore_missing_imports = true`) because the type-check venv (`uv run` with no `--group docs`) lacks the docs dependency. `docs/docs_update.py` then keeps only what can't be a mirror block: the `readme.md` mindmaps (a `:mirror:` block would expose the generator's `{python:render}` fence on the user-facing README) and the reST `{eval-rst}` autodoc regions (autodoc can't be a Markdown mirror). Update those marker regions through click-extra's own `update_blocks(paths, rewrite, *, check=False)` — the walk/write/dry-run primitive behind `:mirror:` and `{matrix}` — instead of a hand-rolled `wcmatch` loop; it hands you a `--check` mode for CI drift detection for free. It is public API in `click_extra.blocks` since click-extra `8.8.0`, which `click_extra.sphinx` also re-exports.
...
- `alert` — renders GitHub-style alerts (`> [!NOTE]`, `> [!WARNING]`, …) as admonitions. Native to `myst-parser` ≥ 5.1, which `click_extra.sphinx` defers to from that version on; every repo in this lineage enables it. Preferred over `colon_fence` for alert rendering.
- `attrs_block`, `attrs_inline` — class/style attributes on blocks and inline spans.
- `colon_fence` — enables `:::`-delimited directives, and GitHub `[!NOTE]` admonitions when paired with `myst_parser` ≥ 3 (though `alert` above is the preferred path for those). This lineage otherwise discourages colon-fence admonitions (`mdformat` churns the colons; see § MyST docstrings and admonitions), so enable it only when a project genuinely uses `:::` syntax.
...
- `substitution` — `{{ name }}` substitutions fed from `myst_substitutions`. Enable only when the project injects Python-computed content into pages.
...
- When the schema is thin (a handful of options), skip the standalone `configuration.md`: fold the `{click:config}` block into `cli.md` as a closing `## Configuration` section. Rendering the same schema on several pages is safe: the generated anchors are per-page heading ids, not global labels.
...
8. **Man pages** section, only when the project declares `click_extra_manpages`: the `--man` option, the roff source from `--help-format man`, the release tarball, and the command a packager runs to regenerate the pages. It pairs with the `man` index page of § Standard page roster.
...
A maintainer/packager-facing page. Every project carries the **dependency graph** here; projects distributed through package managers add their per-channel build instructions on the same page.

**Dependencies graph** — a `## Dependencies` section embedding `assets/dependencies.mmd` via a plain ```` ```mermaid ```` fence with `:align: center`. The `.mmd` is regenerated by `repomatic update-dep-graph` from `uv.lock`; never hand-edit it. One sentence explains the shapes: boxes hold the directly-declared dependencies of each group, drawn as hexagons; transitive dependencies render outside the boxes as ovals.

The graph used to close `install.md`; it moved here so the install page stays end-user-focused while packager-facing material lives on one page. `meta-package-manager` is the reference: its `packaging.md` renders the graph above a full per-distro build catalog.
...
06. `packaging` — Distribution and dependency reference (see § Recipes › `packaging.md`). Every project renders its dependency graph here; projects distributed through package managers add their per-channel build instructions alongside it. Maintainer/packager-facing, so it sits here rather than among the user-facing pages.
...
12. `history` — Optional narrative of how the project got to here, opening on its own star-history chart. Context for maintainers rather than instruction for users, so it sits here, next to the changelog it complements.
...
One flat `{toctree}` works until it does not. Past roughly a dozen entries the sidebar stops being a map and becomes a list to scan, and the reader has no way to tell which pages answer the question they arrived with. So **group a block once it passes ~12 entries, and leave it flat below that.** The threshold is per block, not per site.

The shape a grouped `index.md` takes:

- **The first block takes no caption and holds only entry points** — the two or three pages someone opens before they know what they want. `install` is always one. Everything else earns a caption.
...
- `myst_heading_anchors = 6` paired with `myst_heading_slug_func = "docutils.nodes.make_id"` registers every heading as a resolvable cross-reference target, so in-page `[text](#anchor)` links resolve (and broken ones warn) at build time. Pinning the slug function to docutils' `make_id` keeps MyST anchors identical to the section IDs docutils already emits (`cache.dir` → `cache-dir`), so existing anchor URLs stay stable.
...
- Auto-region markers using bare `<!-- start -->` / `<!-- end -->` instead of named `<!-- {feature}-{kind} -->` / `<!-- {feature}-{kind}-end -->`. Migrate on first touch; the regenerator must be updated in the same commit.
