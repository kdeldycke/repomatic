...
**Hard prerequisite — the generator must be importable from the installed package.** The mirror refresher and the `source-read` rebuild both `exec` each block with a bare namespace and no `sys.path` setup, so a mirror block may only import from the installed distribution (`from {package}.x import ...`), never a docs-local helper. A block that does `from docs_update import ...` raises `ModuleNotFoundError` under `click-extra refresh-directives` run from the repo root (verified in extra-platforms), which fails the whole Phase-4 refresh. Two ways out: (a) ship the generator inside the package; or (b) if the generators are deliberately docs-local (not public API), skip `:mirror:` and keep a Phase-3 refresher in `docs/docs_update.py` that adds `docs/` to `sys.path`, `exec`s each `{python:render}` block, and writes the output into a companion `<!-- {tag}-snapshot -->` HTML-comment region (invisible in the build, inspectable in source). Extra-platforms uses (b) — its `sync_render_snapshots()` — because its table/diagram generators are docs-only tooling that shouldn't enter the library's public surface.
