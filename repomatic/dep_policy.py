...
{mod}`~repomatic.dep_sources` answers "can this ship": a git branch or a local
...
class PolicyFinding:
    ...
    Deliberately not a {class}`~repomatic.dep_sources.DepFinding`: that type
    carries a {class}`~repomatic.dep_sources.SourceKind` because every one of
