...
checksum recompute that the commands drive stay in {mod}`repomatic.cli.main`. The
...
{data}`repomatic.deps.dep_report.EXCLUDE_NEWER_HELD_BACK_NOTE`.
"""
...
def exclude_newer_cutoff(value: str, today: date) -> str | None:
    ...
    {func}`repomatic.tooling.tool_runner.run_tool`) by the same window
...
def format_cooldown_note(age_label: str, cutoff: date) -> str:
    ...
    {func}`repomatic.deps.dep_report.format_exclude_newer_note`. uv records an absolute
    ...
    :return: A one-line markdown note for
        {func}`repomatic.deps.dep_report.format_diff_table`.
    """
    return (
...
def github_candidates(repo_url: str, tag_pattern: str | None = None) -> list[Candidate]:
    ...
    :param tag_pattern: Per-tool version-extraction regex (see
        {attr}`repomatic.tooling.tool_registry.ToolSpec.tag_pattern`).
