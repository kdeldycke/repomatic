...
"""Job-matrix construction of {class}`~repomatic.metadata.Metadata`.
...
class MatrixMetadata:
    ...
    A concern mixin of {class}`~repomatic.metadata.Metadata`: never
    ...
    if TYPE_CHECKING:
        ...
        def config(self) -> Config:
            """See {class}`~repomatic.metadata_project.ProjectMetadata`."""

        @cached_property
        def current_commit_matrix(self) -> Matrix | None:
            """See {class}`~repomatic.metadata_git.GitMetadata`."""
        ...
        def event_type(self) -> WorkflowEvent | None:
            """See {class}`~repomatic.metadata_env.EnvironmentMetadata`."""
