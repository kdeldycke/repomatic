...
class Config:
    ...
    baked into the generated thin caller, and a manual dispatch still overrides
    it.
    """

    notification_months: int = field(
        default=3,
        metadata={CONFIG_PATH_METADATA_KEY: "notification.months"},
    )
    """Inactivity a closed thread must reach before the unsubscribe workflow
    acts on it.
    ...
    upstream_pin_sync: bool = field(
        default=True,
        metadata={CONFIG_PATH_METADATA_KEY: "upstream-pin.sync"},
    )
    """Whether `repomatic init --upgrade` moves this repository to a newer `repomatic`.

    The `sync-repomatic` job passes `--upgrade`, so it opens one pull request
    moving the upstream `uses:` pin to the newest release past the
    `minimum-release-age` cooldown, with every managed file regenerated by that
    release. Set to `false` to keep the job syncing files at the release already
    pinned.
    """
