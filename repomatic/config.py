...
class Config:
    ...
    also gates ad-hoc installs at run time, so their transitive trees honor the
    same window: `repomatic run`'s `uvx` tools (via uv's `--exclude-newer`) and
    the `lint-awesome` npm install (via npm's `min-release-age`). The
