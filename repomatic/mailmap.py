...
def remove_header(content: str) -> str:
    ...
    Strips the metadata block `sync-mailmap` writes at the top of the file
    (`generated_header` output: `# Generated by …` and `# Timestamp: …` lines),
    so a re-run parses only the identity mappings.
    """
    lines = []
    still_in_header = True
    for line in content.splitlines():
        if still_in_header:
            # Still in the header as long as the lines are blank or match the
            # comment format `generated_header` produces.
        ...
        # Past the header: keep every remaining line.
        lines.append(line)
    return "\n".join(lines)
