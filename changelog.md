...
## [`6.25.0.dev0` (unreleased)](https://github.com/kdeldycke/repomatic/compare/v6.24.0...main)

> [!WARNING]
> This version is **not released yet** and is under active development.

- Propagate the `UV_OVERRIDE=uv-overrides.txt` click override into Renovate's `postUpgradeTasks` by prefixing the `uvx --from . repomatic update-checksums` invocations in `renovate.json5` (and the bundled `repomatic/data/renovate.json5` mirror). Workflow-level `env:` does not reach Renovate's child processes, so the bare command hit the `bump-my-version<8.4` vs `click-extra>=8.4.1` resolver deadlock and posted an "Artifact update problem" comment on every PR that touched a workflow file. The `RENOVATE_ALLOWED_COMMANDS` regex in `renovate.yaml` gains an optional `UV_OVERRIDE=uv-overrides\.txt ` alternative before `/tmp/uvx` so both shapes (with and without the env prefix) remain allowed.
- Add man page generation to the release and docs pipelines. A new `manpages` job in `_release-engine.yaml`, activated by `[tool.repomatic.manpages]` config keys (`script`, `asset-name`) in `pyproject.toml`, shells out to `click-extra man --output-dir man "${SCRIPT}"` and uploads the rendered roff `.1` files as a `<asset-name>.tar.gz` (defaulting to `<package-name>-manpages.tar.gz`) on the GitHub release; requires `click-extra>=7.19`. Install `mandoc` alongside Graphviz in the `deploy-docs` job so projects using `click_extra.sphinx.manpages` get browser-viewable `.html` siblings next to the roff files in the docs build: without a roff renderer on `PATH`, the Sphinx hook silently skips the HTML pass and index links 404. Both are silently skipped when `manpages.script` is unset.
...
- Improve GitHub failure detection across `repomatic.github`: annotate `gh` and PAT permission check failures with the current [githubstatus.com](https://www.githubstatus.com) summary (memoized per process, 3-second timeout, no-op when the status page is unreachable); broaden the `GITHUB_TOKEN` fallback in `run_gh_command` to fire on `Requires authentication` in addition to `Bad credentials`, so a 401 from the primary token degrades to the ambient Actions token instead of hard-failing; rewrite PAT permission check failure messages to surface raw stderr for non-403 failures instead of misreporting them as missing scopes.
...
- Tighten `/repomatic-ship`'s pre-push gate and reconciliation sweep. The gate now runs both `ruff check` and `ruff format --check`, mirroring CI's `format-python` job: a format-only violation (blank-line count, trailing whitespace) that `check` alone misses no longer slips through to an unattended autofix PR after the push. The gate also smoke-runs `uvx --no-progress --from . repomatic --version` to surface dependency-resolution failures that `uv run` (which resolves against the already-synced venv) cannot: a transitive conflict introduced by a same-cycle dependency bump manifests only when `uvx` builds a fresh isolated environment from `[project.dependencies]`, which is exactly what CI's `🧬 Project metadata` job does on every workflow. The skill now documents the `UV_OVERRIDE=uv-overrides.txt` workaround for cases where `[tool.uv] override-dependencies` (not read by `uvx --from .`) is insufficient. The sweep instructions also call out that the code and docs agents must be spawned in a single tool-call block; spawning them in separate messages serializes them and wastes the wall-clock of whichever pass finishes first.
- Replace the `tomlkit` dependency with [`tomlrt`](https://dimbleby.github.io/tomlrt/) for all comment-preserving TOML writes. Extend `tomlrt` to all TOML reads too (`pyproject.toml`, `uv.lock`, `[tool.X]` extraction), dropping the `tomli` backport pulled in only on Python 3.10 and the conditional `tomllib`/`tomli` import shim previously scattered across eight modules.
