...
- **Breaking:** the `agents` component is renamed `subagents`, and its config key `agents.location` becomes `subagents.location`. A repository naming the old component in `[tool.repomatic] include` or `exclude` fails to init until it is updated; the old config key is ignored with an unknown-option warning, silently returning subagents to the default directory.
- **Breaking:** `repomatic init claude` is renamed `repomatic init agent`, and its destination is now `[tool.repomatic] agent.location` rather than a fixed root-level `claude.md`. It follows `[tool.repomatic.flavor] agent` when unset, so a repository keeping its instructions as `AGENTS.md`, or outside the root, is now reachable by the sync instead of getting a second document beside the one it already had.
...
- **Breaking:** the Cloudflare Pages deploy needs only `CLOUDFLARE_API_TOKEN`. A token scoped to `Cloudflare Pages: Edit` resolves its own account, so `CLOUDFLARE_ACCOUNT_ID` becomes an override for a credential spanning several accounts, and `lint-repo` and the setup guide stop asking for it.
- New `cloudflare-pages --account-id` printing the resolved account identifier, bare enough to pipe straight into `gh secret set`, for the multi-account case that still wants it.
- New `cloudflare-pages --attach-domain` serving a project at a custom domain, creating the proxied `CNAME` alongside it: attaching a hostname over the API provisions no DNS, unlike the dashboard, so the domain stays `pending` with nothing saying why.
...
- New Cloudflare Pages documentation page covering Direct Upload, token scoping and rotation, the drift check, the redirects engine's undocumented accounting, and how a GitHub Pages custom domain keeps old `github.io` URLs redirecting after the move. It opens on a runbook taking a bare domain name to a published site in seven commands.
- This project's own documentation moves to [repomatic.net](https://repomatic.net), and its links move with it: every reference now names the extensionless URL Cloudflare Pages serves, resolving in one hop instead of three. The old `kdeldycke.github.io/repomatic` addresses keep redirecting.
...
- New `sample-metrics` command recording what forges say about the repositories a project tracks, into one committed CSV. Reads GitHub, GitLab and Forgejo instances for each subject's star count, newest release or tag, and newest commit. A counter accrues one row per day so its curve can be charted; an attribute keeps a single row, restamped only when its value moves.
- `sample-metrics` renders the accumulated history as themeable SVG charts, replacing the third-party star-history embeds GitHub's 2026 stargazer restriction left rendering an error card. A GitHub repository the token administers is reconstructed exactly from its per-star timestamps; the rest are sampled forward, backfilled from archived pages with `--backfill-wayback`, or imported from a star-history.com export with `--import-csv`.
- New `[tool.repomatic.metrics] charts` option `scale = "logarithmic"`, measuring the vertical axis in powers of ten so series orders of magnitude apart stay legible on one chart. Independent of `mode`, which measures the horizontal one.
- A chart no longer clips its longest series label: the right margin is measured from the labels rather than fixed at a width that suited a single-series chart.
- `sample-metrics --backfill-wayback` now animates a spinner naming the capture it is reaching for and prints each recovered point, instead of running silent for the minutes it spends per repository.
