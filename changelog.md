...
## [`6.25.2.dev0` (unreleased)](https://github.com/kdeldycke/repomatic/compare/v6.25.1...main)
...
- Add `[tool.repomatic.labels.file-rules]` and `[tool.repomatic.labels.content-rules]` config for structured per-label labeller rules. Each `[[file-rules]]` entry is one match group with `label` plus any combination of `actions/labeler` v5+ matchers: `any-glob-to-any-file`, `any-glob-to-all-files`, `all-globs-to-any-file`, `all-globs-to-all-files`, `head-branch`, `base-branch`, and the `any` / `all` group wrappers (nested recursively). Repeating a label across entries OR's the resulting groups. Each `[[content-rules]]` entry is `{label, patterns}` for `github/issue-labeler`. Both are serialized to YAML at `repomatic init labels` and appended to the bundled `labeller-file-based.yaml` / `labeller-content-based.yaml`.
- Remove `labels.extra-file-rules` and `labels.extra-content-rules` config fields. They had been silently ignored since the `gha-utils → repomatic` rewrite, and the new `labels.file-rules` / `labels.content-rules` cover the full `actions/labeler` v5+ schema natively in TOML, removing the need for a raw-YAML escape hatch in `pyproject.toml`.
