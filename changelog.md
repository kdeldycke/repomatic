...
- New `[tool.repomatic] sphinx.deploy` config choosing where the Docs workflow publishes the built site: `github-pages`, the default, or `cloudflare-pages` through `wrangler pages deploy`. The Cloudflare path reads the optional `CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ACCOUNT_ID` secrets, and runs as a job of its own so each target keeps only the permissions its host needs.
