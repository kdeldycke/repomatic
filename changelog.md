...
- Fix `_release-engine.yaml` emitting the string `"null"` (via `toJSON(null)`) for the `release_commits_matrix` output on non-release pushes. That string is truthy in GitHub Actions expressions, so `release.yaml`'s `|| '{"include":[]}'` fallback never fired; `fromJSON("null")` returned JSON null which GitHub can't use as a matrix definition, failing every non-release run with `Unexpected value ''`. The output now emits an empty string when the matrix is null, restoring the intended fallback.
