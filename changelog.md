...
- Tighten `/repomatic-ship`'s pre-push gate: add `ruff format --check` alongside `ruff check`, and smoke-run `uvx --no-progress --from . repomatic --version` to surface dependency-resolution failures before CI.
