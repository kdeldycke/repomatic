...
- Read the standard `[tool.nuitka]` section from `pyproject.toml` and translate it into Nuitka CLI flags for the onefile build, via `nuitka_args_from_pyproject` in `repomatic.binary` (`key = true` → `--key`, `key = "value"` → `--key=value`, `key = ["a", "b"]` → a repeated `--key=`, `key = false` skipped). Nuitka itself reads `[tool.nuitka]` only when it acts as the build backend (setuptools wheel builds), not for the CLI onefile builds driven here ([Nuitka#2136](https://github.com/Nuitka/Nuitka/issues/2136)), so projects previously could not configure the binary through the canonical section. The derived flags are prepended to `[tool.repomatic] nuitka.extra-args` and the auto-detected `--python-flag=-m`.
- Serialize the Nuitka build flags from the `metadata` job as a JSON array and rebuild the argv list in `release.yaml` with `jq`, replacing the space-joined string that the build step expanded unquoted. Flag values containing spaces (like `--product-name=Meta Package Manager` or a `--copyright` notice) were previously word-split and impossible to pass; they now reach Nuitka intact.
