...
## [`7.9.0.dev0` (unreleased)](https://github.com/kdeldycke/repomatic/compare/v7.8.0...main)
...
- Release-commit builds bypass the compile cache and macOS builds disable ccache entirely, so no published binary is influenced by cached objects or by Nuitka's unchecksummed ccache download.
...
- `sync-gitignore` no longer discards rules added by hand to the committed `.gitignore`. It rebuilds the file from gitignore.io plus `gitignore.extra-content` and never reads the copy on disk, so anything appended there was dropped on the next run with nothing to warn that the sync pull request was undoing a deliberate decision. An audit of a downstream blog found it one run away from losing the entry ignoring its 194 MB build directory. The command now compares the rule sets first and refuses to write when the rebuild would lose one, naming the rules and exiting non-zero. New `--drop-orphans` flag confirms the loss when it is intended. Comments and ordering are excluded from the comparison, so a reformatted file is not read as a rewritten one.
- The setup guide no longer asks a uv virtual project to register a PyPI Trusted Publisher, and `lint-repo` no longer checks one. Both were gated on `[project] name` being set, which such a project declares purely to carry its dependencies, so a blog or documentation site was sent to claim a PyPI name it does not own, authorizing a `release.yaml` that `RepoScope.PACKAGE_ONLY` never generates for it. The close gate read the same name, which made the guide unresolvable: it kept demanding upload provenance no workflow in the repository can produce, so the issue stayed open after every other step had passed. Both now resolve `is_python_package`, the predicate the release lane is already scoped on, exposed as `Metadata.is_python_package`.
- `sync-workflow-pins` now walks `npx tool@1.2.3` version literals forward, not just `npm install`. `npx` is the usual spelling in a workflow, since it runs a tool without installing it, and its pins were the one kind nothing kept current: a pinned `npx --yes html-validate@10.1.1` step sat frozen while every action SHA and `uvx` pin around it moved. Flags between the command and the package (`--yes`, `--quiet`) are skipped over, and scoped names are matched.
