...
## [`6.22.0.dev0` (unreleased)](https://github.com/kdeldycke/repomatic/compare/v6.21.0...main)

> [!WARNING]
> This version is **not released yet** and is under active development.

- `repomatic init` now prunes downstream orphans of skills, agents, and workflows that repomatic has renamed or removed. An on-disk copy whose content matches any version repomatic shipped for that asset is deleted, so an upstream rename (such as `repomatic-release` becoming `repomatic-ship`) propagates automatically instead of leaving a stale file behind; a copy edited locally is reported for manual review, never deleted. The seed list covers every skill dropped across the project's `gha-utils` → `repokit` → `repomatic` renames, so a repo first set up under an earlier name sheds its legacy skill directories on the next run. The `sync-repomatic` autofix job applies this on every push. Pass `--keep-removed` to report orphans without deleting them, or `--delete-removed-modified` to also delete locally modified ones.
