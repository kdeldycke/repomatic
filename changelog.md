...
- Annotate `gh` and PAT-permission-check failures with the current [githubstatus.com](https://www.githubstatus.com) summary so a GitHub-side incident surfaces in the error message instead of sending operators to chase PAT scopes. The probe is memoized per process, capped at a 3-second timeout, and collapses to a no-op when the status page itself is unreachable.
- Broaden the `GITHUB_TOKEN` fallback in `repomatic.github.gh.run_gh_command` to fire on `Requires authentication` (a GitHub auth incident or a fine-grained PAT scope mismatch GitHub treats as anonymous for that resource) in addition to `Bad credentials` (expired or revoked PAT), so a 401 from the primary token degrades to the ambient Actions token instead of hard-failing.
- Rewrite the failure messages emitted by `repomatic lint-repo`'s PAT permission checks (`Contents`, `Issues`, `Pull requests`, `Workflows`, `Commit statuses`, `Dependabot alerts`): an unambiguous `HTTP 403` still recommends adding the missing permission, but any other failure (a transient `401`, a `5xx`, a network error) now surfaces the raw stderr instead of mis-reporting it as a missing scope.
