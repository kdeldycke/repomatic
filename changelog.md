...
- Add `_release-manpages.yaml` reusable workflow: renders a Click CLI command tree as roff `.1` files and uploads them as a `<asset-name>.tar.gz` asset on the GitHub release. Sibling of `_release-build.yaml` and `_release-engine.yaml`; consumers opt in by adding a 5-line `uses:` block to their `release.yaml`. Uses `click_extra.man_page.write_manpages()` against the consumer's already-synced venv (no extra resolver install), so any project on `click-extra>=7.18` can adopt it without waiting for the newer `click-extra man --output-dir` CLI flag. See `docs/workflows.md` for the wiring snippet.
