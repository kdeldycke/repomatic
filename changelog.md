...
- Switch `[tool.typos]` to `sync_mode=ONGOING` so the bundled proper-noun identifiers (`GitHub`, `macOS`, `PyPI`, and the rest) reach repos that already declare their own `[tool.typos]`. A `BOOTSTRAP` insert skipped any existing section, and because typos reads `[tool.typos]` natively there is no runtime fallback, so such repos enforced none of the canonical capitalizations. The ongoing-sync merge now grafts local additions back over the template for nested and inline tables (like a downstream key in `default.extend-identifiers`), not only `[[files]]` arrays-of-tables; the template wins on shared scalars, `preserved_keys` still overrides per key, and rebuilt inline tables use `pyproject-fmt` spacing so they stay format-stable.
- Carry the two repomatic-specific `[[tool.bumpversion.files]]` pyproject.toml entries in the bundled `bumpversion.toml` template so downstream Python repos receive them: the `[project]` version search is anchored to a line start (`(?m)^version = `) so it cannot rewrite version-suffixed keys like `[tool.nuitka]`'s `file-version`/`product-version`, and a dedicated rule keeps those numeric keys in sync with `ignore_missing_version` set (a no-op where no `[tool.nuitka]` exists). The ongoing-sync graft gains a `graft_identity_keys` slot identity (file target plus `replace`): a local `[[files]]` entry sharing a template entry's slot is now treated as a stale copy the template supersedes, rather than appended as a duplicate, so a repo carrying the previous unanchored `[project]` entry converges on the single anchored one instead of ending up with both (which would break the version bump).
- Run Nuitka through the tool runner: `repomatic run nuitka` installs the pinned Nuitka, prepends `--mode=onefile`, and reads the standard `[tool.nuitka]` section from `pyproject.toml`. A new tool-runner translation target (`NativeFormat.FLAGS`, backed by `pyproject_table_to_flags`) translates a `[tool.X]` table into CLI flags (`key = true` → `--key`, `key = "value"` → `--key=value`, `key = [...]` → a repeated flag, `key = false` skipped) for tools whose CLI cannot read `[tool.X]` itself. Nuitka only consults the section natively when acting as the wheel build backend, not for the CLI onefile builds run here ([Nuitka#2136](https://github.com/Nuitka/Nuitka/issues/2136)). `release.yaml` builds via `repomatic run nuitka` instead of invoking Nuitka directly, and Nuitka now appears in `repomatic run --list` and the tool-runner docs. Nuitka is invoked as `python -m nuitka` rather than via its console script because Nuitka 4.1 installs only a `.cmd` wrapper on Windows — no `.exe` launcher — so `uv run -- nuitka` cannot locate it. A new `module` field on `ToolSpec` enables this per-tool override; its integrity constraint requires `needs_venv=True`.
- **Breaking:** remove the `[tool.repomatic] nuitka.extra-args` option. Configure binary builds through the standard `[tool.nuitka]` section instead: each flag becomes a key, so `--include-data-files=SRC=DEST` becomes `include-data-files = ["SRC=DEST"]`. The section is now read by the tool runner, making the dedicated option redundant.
- Add binary metadata to the Nuitka build through `[tool.nuitka]`: `product-name`, `file-description`, `copyright`, numeric `file-version`/`product-version` (a dedicated `[tool.bumpversion]` rule keeps them in sync while stripping the PEP 440 `.devN` suffix Nuitka rejects), and `linux-icon`/`macos-app-icon`/`windows-icon-from-ico` pointing at native icon files (`docs/assets/icon.png`/`icon.icns`/`icon.ico`, all rendered from `favicon.svg`). Native formats are required: Nuitka would otherwise need the `imageio` package to convert a non-native image and fail the build.
