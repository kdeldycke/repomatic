...
- New `fix-awesome-toc` command deletes the table-of-contents entries awesome-lint forbids from `readme.md` and from every `readme.{lang}.md` beside it. A translated section is now matched by its position rather than by its English name, so translations stop keeping the entries the English readme loses.
...
- Every GitHub release now carries an attested `repomatic-plugin.zip` asset, which is what the plugin marketplace installs from. New `pack-plugin` command builds it, and the marketplace URL is pinned to the release tag, so adding the catalog at `kdeldycke/repomatic@vX.Y.Z` installs that version's plugin.
- New `init plugin` component merges the plugin's marketplace and enablement keys into a repository's `.claude/settings.json`, leaving every other setting untouched. Opt-in, like `skills` and `agents`. New `[tool.repomatic] settings.location` moves the destination.
- A failed `extra-assets` job now leaves the GitHub release as a draft instead of publishing it without the assets the repository declared. Once published a release is immutable, so a missing asset could never be added. A skipped job still publishes, so repositories declaring no extra assets are unaffected.
...
- The generated `release.yaml` no longer copies a `needs:` edge pointing at a job that exists only upstream, which GitHub rejected at workflow startup.
- Fix `init` downgrading the upstream `uses:` pins of a repository that adopted a release still inside the `minimum-release-age` window. The cooldown now gates an adoption only: a pin equal to the running version is left untouched without a datasource lookup, a step-back never lands below the pin already committed, and a rollback to an older repomatic is honored. `sync-repomatic` was re-judging that verdict on every push, dragging every thin caller back below the version the repository had deliberately moved to, and pointing callers generated from the running release at an older reusable-workflow surface.
