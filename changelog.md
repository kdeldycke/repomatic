...
- New `[tool.repomatic] sphinx.deploy` config choosing where the Docs workflow publishes the built site: `github-pages`, the default, or `cloudflare-pages` through `wrangler pages deploy`. The Cloudflare path needs the `CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ACCOUNT_ID` secrets, and runs as a job of its own so each target keeps only the permissions its host needs.
- The setup guide issue and `lint-repo` now ask about whichever host `sphinx.deploy` names: a Cloudflare-targeting repository is walked through its two credentials instead of being told to set a GitHub Pages source it never uses.
