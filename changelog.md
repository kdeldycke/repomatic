...
- Fix `uvx --from .` dependency conflict between `bump-my-version>=1.2.6` (which caps `click<8.4`) and `click-extra>=7.18.0` (which requires `click>=8.4.1`). The `[tool.uv] override-dependencies` setting is not read by `uvx --from .` (tool invocations are isolated from project settings), so all workflows that invoke `uvx --no-progress --from . repomatic ...` now propagate the override via `UV_OVERRIDE=uv-overrides.txt`. The override file is committed at the repo root; CI picks it up after `actions/checkout`. This fixes the "No solution found when resolving tool dependencies" error that was failing every workflow's `🧬 Project metadata` gate and blocking all downstream jobs.
...
- Tighten `/repomatic-ship`'s pre-push gate and reconciliation sweep. The gate now runs both `ruff check` and `ruff format --check`, mirroring CI's `format-python` job: a format-only violation (blank-line count, trailing whitespace) that `check` alone misses no longer slips through to an unattended autofix PR after the push. The sweep instructions also call out that the code and docs agents must be spawned in a single tool-call block; spawning them in separate messages serializes them and wastes the wall-clock of whichever pass finishes first.
