...
- Add a `manpages` job to `_release-engine.yaml` and matching `[tool.repomatic.manpages]` config keys (`script`, `asset-name`). When a consumer sets `manpages.script` in `pyproject.toml`, the engine renders the resolved Click command tree as roff `.1` files and uploads them as a `<asset-name>.tar.gz` (defaulting to `<package-name>-manpages.tar.gz`) on the GitHub release. Uses `click_extra.man_page.write_manpages()` against the consumer's already-synced venv, so any project on `click-extra>=7.18` opts in without waiting for the newer `click-extra man --output-dir` CLI flag. Silently skipped when `manpages.script` is empty, keeping the job invisible for projects that have not opted in.
