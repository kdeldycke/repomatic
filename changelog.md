...
- Improve GitHub failure detection across `repomatic.github`: annotate `gh` and PAT permission check failures with the current [githubstatus.com](https://www.githubstatus.com) summary (memoized per process, 3-second timeout, no-op when the status page is unreachable); broaden the `GITHUB_TOKEN` fallback in `run_gh_command` to fire on `Requires authentication` in addition to `Bad credentials`, so a 401 from the primary token degrades to the ambient Actions token instead of hard-failing; rewrite PAT permission check failure messages to surface raw stderr for non-403 failures instead of mis-reporting them as missing scopes.
