...
- Add a `manpages` job to `_release-engine.yaml` and matching `[tool.repomatic.manpages]` config keys (`script`, `asset-name`). When a consumer sets `manpages.script` in `pyproject.toml`, the engine shells out to `click-extra man --output-dir man "${SCRIPT}"` against the consumer's already-synced venv and uploads the rendered roff `.1` files as a `<asset-name>.tar.gz` (defaulting to `<package-name>-manpages.tar.gz`) on the GitHub release. The job requires `click-extra>=7.19`, which ships the `--output-dir DIR` option that writes one `.1` file per resolved (sub)command into `DIR`, creating the directory if missing. Silently skipped when `manpages.script` is empty, keeping the job invisible for projects that have not opted in.
- Install `mandoc` alongside Graphviz in the `deploy-docs` job of `docs.yaml`, so projects opting into `click_extra.sphinx.manpages` get the browser-viewable `.html` siblings rendered next to the roff `.1` files during the docs build. Without a roff renderer on `PATH`, the hook skips the HTML pass silently and the man-page index links 404 on the deployed site. Projects that do not enable the hook are unaffected.
