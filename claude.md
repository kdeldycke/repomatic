...
- **A test whose subject is the rendering must not fetch the data live, and check which way the default falls.** Serialization, table formatting, file destinations and column projection are all assertions about a *writer*: they need rows, not the host's real ones, so they take the stub. Only a test whose subject is the data itself keeps the live source, and one such case per family is the budget, never one per format. What makes this bite is the direction of the default: a test usually acquires the expensive, host-dependent path by *omitting* a fixture, so the slow path is what an author lands on by accident, then gets multiplied by every parametrize case and every subclass inheriting it. `meta-package-manager` paid for that three times over, worst of all in a fifty-format rendering test inherited by six subcommand classes: three hundred real-subprocess invocations, two hundred and fifty of them byte-identical, and nothing in the run reported it, because duplicate work looks exactly like coverage.
- **A conformance test guards a construct, not a defect class.** After writing one, ask where else the same mistake can live, because the guard only walks what you pointed it at. The rendering-versus-data rule above was enforced over the methods of the template classes, and the identical defect was sitting in a module-level parametrize the guard never visited; profiling found it, not the test. Either widen the guard to every construct the defect fits, or say in its docstring which shapes it does not cover, so the next reader does not mistake a narrow check for a complete one.
...
- **A file you stage may carry a concurrent process's lines.** Several agents working one checkout write to the same `changelog.md` or `pyproject.toml`, so `git status` naming a file is not proof the diff is yours: read `git diff --cached` on every staged file before committing, and stage hunks selectively when part of a file belongs to in-flight work elsewhere. If a foreign line slipped into an unpushed commit anyway, amend it out and restore it to the working tree for its owner, never delete it.
