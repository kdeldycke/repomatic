...
- **No decorative prefixes.** This is not [Conventional Commits](https://www.conventionalcommits.org): no `feat:`, `chore:`, `fix:`. A `[bracketed]` prefix is reserved for the machine-parsed release contract (`[changelog] Release vX.Y.Z`, `[changelog] Post-release bump …`), which {data}`repomatic.git_ops.VERSION_BUMP_COMMIT_PREFIXES` and the auto-tagging job match literally. Inventing another one is noise at best and a collision at worst.
