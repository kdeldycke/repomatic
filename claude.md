...
5. **A read-only command may expose mutation via `--fix`.** When a query command (read-only by default, like `audit`) gains an autofix mode behind a `--fix` flag, the autofix workflow operation keeps its own `fix-X` job ID, PR branch, and PR body template name and invokes `<command> --fix` (the `fix-vulnerable-deps` job runs `audit --fix`). The command name is then exempt from rule 3's four-way agreement: it is a general-purpose query command (like `metadata`), not the operation's namesake.
...
- **Cover the shipped config broadly; probe unreleased axes narrowly; smoke-test released flavors.** Released dependencies on stable Python get the full cross-platform spread (what users install). Unreleased dependency branches and prerelease Python run on one runner as `continue-on-error` early-warning probes (via `test-matrix.unstable`), never across platforms. A released free-threaded build (`3.14t`) is a flavor of an already-broadly-tested base version, so it runs **stable** (expected to pass) on a single runner, not as an `unstable` probe: narrow spread, stable outcome. Express it as a `python-version` variation pinned with `exclude` and left out of `unstable`.
...
- **Select runners by measured speed and workload, not architecture.** Read your own CI job timings and keep the faster architecture per OS; do not default to ARM as "newer." The ARM advantage is workload-dependent: the parallel `pytest --numprocesses=auto` suite runs ~2-3x faster on `ubuntu-24.04-arm` than the lean x86 `ubuntu-slim`, so the test PR Linux slot uses ARM; but setup-bound light jobs (checkout + `uv install` dominated) barely move, so they keep the lean `ubuntu-slim` default. Whenever one fast runner suffices (PR Linux slot, a single-runner flavor smoke test like `3.14t`, a pinned dependency cell), `ubuntu-24.04-arm` is the default pick: fastest measured on compute-bound parallel work and the cheapest tier (hosted macOS bills ~10x Linux minutes), so `macos-26`/Windows are reserved for the OS coverage only they add. When isolating architecture, A/B a full-x86 image against ARM (not the lean image), or you credit the arch for the image's leanness. Other axes: Apple-silicon macOS beats Intel ~2x; on Windows the two tie on compute and `windows-2025` (x86) only wins per-job from a systematic ~50s `windows-11-arm` Codecov-upload penalty. Drop the slower twin via `test-matrix.remove.os` when both are not needed.
...
- **`repomatic run {tool} --check` is unreliable for tools with a post-process fixup.** A few tools (currently `mdformat`) get a Python post-processing pass after the formatter exits, and that pass only runs in write mode. So `--check` can report drift the write path would silently reconcile (false positive), or pass on files the write path would still rewrite (false negative). `repomatic run` warns when a check flag meets such a tool, but the rule stands: to verify or gate formatting, run the write path and inspect `git diff`, never `--check`.
...
Downstream Python repos should pin uv in `[tool.uv]` to a bounded range (like `required-version = ">=0.11.23,<0.12"`) rather than leaving it floating. `astral-sh/setup-uv` reads this value automatically: its `version` input defaults to the project's `required-version`, falling back to latest only when unset. So one pyproject key makes every CI runner and every contributor's local uv resolve `uv.lock` with the same resolver, and uv enforces the constraint on each invocation.

This closes the second half of the `sync-uv-lock` churn problem. `sync_uv_lock` already discards a re-lock that moved no package versions, because uv re-spells semantically equivalent environment markers differently across versions and resolution modes (flipping a transitive dependency's `python_full_version < '3.13'` to the equivalent `< '3.11'`, or back, when the parent is only reachable below 3.11). Pinning uv removes the version skew that produced those re-spellings in the first place, so neither CI nor local development reintroduces the drift. Prefer a bounded range over `==`: it tolerates patch updates while holding the resolver-significant minor. Since `required-version` is a hard gate (uv refuses to run when the installed uv falls outside it), bump the ceiling only once a new uv minor has regenerated the lock.

repomatic keeps the pin current for you, so downstream repos do not hand-maintain it: `repomatic init uv` writes both policy pins (`required-version` and `exclude-newer`) into `[tool.uv]` from the bundled `uv.toml` template, and the `sync-uv-lock` job re-applies them on every run while leaving every other `[tool.uv]` key the project owns untouched. A ceiling bump in the canonical template therefore reaches downstream repos through the normal sync, not a hand edit.
