...
"""Every audience a section may declare, per `claude.md` § Section audience tags.

`downstream` has no members yet: the sections a repo needs *because* it consumes
repomatic (which workflow content it owns, how to move the pin, what
`REPOMATIC_PAT` needs) are today hand-written in each repo rather than shipped.
The value is defined so that content has somewhere to land.
"""
...
"""Scope qualifiers, mirroring the {class}`~repomatic.registry.RepoScope` subset
a section can usefully narrow to. `all` is the default and is written by omission.
"""
...
class Section:
    """One `claude.md` heading and the audience it declares."""
    ...
    def reaches_downstream(self) -> bool:
        """Whether `repomatic init` would write this section into another repo."""
        return self.audience in {"all", "downstream"}


def parse_sections() -> list[Section]:
    """Every heading in `claude.md` below the document title, with its tag.

    ```{note}
    This parser belongs in the package once a `claude` component exists to
    consume it, alongside the `lint-repo` drift check reading the same shape.
    It sits here while the tags are the only consumer.
    ```
    """
    ...
    for index, line in enumerate(lines):
        if line.lstrip().startswith("```"):
            ...
            continue
        if in_fence or not (heading := HEADING_RE.match(line)):
            continue
        level = len(heading.group("level"))
        title = heading.group("title").strip()
        # The document title carries no tag: it is the file, not a section.
        if level == 1:
            continue
        while stack and stack[-1][0] >= level:
            stack.pop()
        audience, scope = "", "all"
        # The tag sits in the blank-line-separated block right under the
        # heading, so a short lookahead is enough and never reaches the body.
        for candidate in lines[index + 1 : index + 4]:
            if tag := TAG_RE.match(candidate.strip()):
                audience = tag.group("audience")
                scope = tag.group("scope") or "all"
                break
        sections.append(
            Section(level, title, audience, scope, stack[-1][1] if stack else "")
        )
...
section_param = pytest.mark.parametrize(
    "section", SECTIONS, ids=[section.title for section in SECTIONS]
)
...
def test_subsection_never_outreaches_parent(section: Section) -> None:
    ...
    if not section.parent:
        return
    parent = SECTIONS_BY_TITLE[section.parent]
    assert not (section.reaches_downstream() and not parent.reaches_downstream()), (
...
def test_downstream_asset_cites_no_upstream_section(
    ...
    offenders = sorted(
        ...
        and not SECTIONS_BY_TITLE[title].reaches_downstream()
