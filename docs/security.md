...
The [`scan-virustotal`](#github-workflows-release-engine-yaml-jobs) job in `_release-engine.yaml` uploads all compiled binaries to [VirusTotal](https://www.virustotal.com/) on every release. This seeds AV vendor databases to reduce false positive rates for downstream distributors (Chocolatey, Scoop, etc.).
