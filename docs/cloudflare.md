...
A repository that moves from `site.deploy = "github-pages"` to `cloudflare-pages` strands every URL already published under `https://<owner>.github.io/<repo>/…`: search indexes, other projects' readmes, packaging metadata and years of issue comments keep pointing there. Nothing on the Cloudflare side can ever answer for them, because the old hostname is GitHub's: no DNS record is yours to change, GitHub Pages emits no server-side redirects, and it reads no `_redirects` file. The old host has to keep answering, and the only thing it can answer with is HTML.

The recipe is a stub tree, deployed to GitHub Pages one final time:

1. One stub per previously published URL, redirecting to its new address with `<meta http-equiv="refresh" content="0; url=…">`. Search engines treat an instant meta refresh as a permanent redirect, so the indexed URLs transfer instead of dying. Each stub also carries a `<link rel="canonical">` naming the new URL, and a visible link for whatever renders HTML but honours no refresh. The previous deployment's `sitemap.xml` is the inventory of what to stub, and when the move coincides with a `sphinx.builder` switch from `html` to `dirhtml`, the stub mapping is where the path shape changes: `/page.html` on the old host, `/page/` on the new one.
2. A `404.html` at the tree's root, catching the long tail with a script rewriting `location` onto the new host, path preserved. GitHub Pages serves it for every path no stub covers. A safety net for humans only: the response status stays 404, which tells a crawler nothing, so it complements the stubs rather than replacing them.

Ship the tree through the same artifact pair the Docs workflow used (`actions/upload-pages-artifact` then `actions/deploy-pages`) in a one-off dispatch, and delete whatever throwaway workflow carried it: GitHub Pages keeps serving its last deployment indefinitely, and with `site.deploy` now naming Cloudflare, the Docs workflow's GitHub Pages job is gated off and can never overwrite the stubs. Two ways to lose them anyway, both avoidable: disabling GitHub Pages in the repository settings deletes the deployment outright, so leave Pages enabled with its source on GitHub Actions; and a `gh-pages` *branch* is the wrong vehicle for the stubs, both because flipping the source away from GitHub Actions is a settings change nothing tracks and because `lint-repo` flags stale `gh-pages` branches, which the stub branch would immediately be.

The move is finished only when the repository stops advertising the old host in its own metadata: the GitHub homepage field, `[project.urls]`, badges and install guides all carry absolute self-links, and `lint-repo`'s website check is what catches the ones forgotten. Once traffic lives on Cloudflare, any *further* URL move is the [redirects engine's](#the-redirects-engine-as-it-actually-is) job: the stub tree answers for the old host, `_redirects` for the new one, and neither can do the other's work.
