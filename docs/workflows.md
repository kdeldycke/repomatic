...
- An upgrade is proposed only for an image this repository does not run yet. A successor already in the matrix is skipped: its `unstable` entry marks every cell on that image `continue-on-error`, so one job left pinned to an older image would otherwise stop the current image from gating the build
