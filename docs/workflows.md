...
jobs:
  lint:
    uses: kdeldycke/repomatic/.github/workflows/lint.yaml@v6.24.0
```
...
- Detects vulnerable packages from two advisory sources, unioned and deduplicated per package by advisory identity (a shared `advisory_id` or a cross-referenced CVE/GHSA/PYSEC alias):
...
- **Requires**:
  - Python package (with a `pyproject.toml` file)
  - `uv` >= `0.11.15`, for the `uv audit --output-format json` output the scan parses (an older `uv` raises rather than silently scanning nothing)
