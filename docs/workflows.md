...
- Its pull request body points at the [`/repomatic-upgrade`](agent-skills.md) skill, which reviews what a newly adopted `repomatic` release lets the repository adopt, reuse or drop. A manual `repomatic init` that moves the upstream pin prints the command launching that review as its last next step
