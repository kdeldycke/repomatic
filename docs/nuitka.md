...
- [Nuitka#3879](https://github.com/Nuitka/Nuitka/issues/3879): a `__main__.py` entry point must be compiled as its package directory plus `--python-flag=-m`, or the binary silently exits. {py:attr}`Metadata.nuitka_matrix <repomatic.metadata.matrix.MatrixMetadata.nuitka_matrix>` computes the flag per entry point, and substitutes the package directory for the `__main__.py` path, without which the package's own modules join `sys.path` and shadow the standard library. On `4.2` the alternative does work for this project: `--main-entry-point` paired with the undocumented `--project-name` builds a binary that runs. It is not yet a safe swap for downstream repos: when the console script carries its package's own name, the generated launcher shadows that package and the binary dies on startup ([Nuitka#4024](https://github.com/Nuitka/Nuitka/issues/4024)). repomatic escapes it only because its builds run from the checkout, where the package sits in the build directory; a repo compiling from an installed distribution does not.
- [Nuitka#3909](https://github.com/Nuitka/Nuitka/issues/3909): Nuitka `4.2` reads `[tool.nuitka]`, but only under `--project`, a mode that takes its entry points from the project configuration and refuses a positional main program. repomatic names its main module on the command line, so the [tool runner](tool-runner.md#nuitka) keeps translating the section into CLI flags at build time. Three upstream defects block the swap even for a project willing to restructure its entry point: `--project` needs `uv_build` inside Nuitka's own environment ([Nuitka#3967](https://github.com/Nuitka/Nuitka/issues/3967)), warns on four standard `[tool.uv]` keys ([Nuitka#3968](https://github.com/Nuitka/Nuitka/issues/3968)), and aborts on this project's `py.typed` and on its `include-package-data` for a dependency ([Nuitka#4025](https://github.com/Nuitka/Nuitka/issues/4025)).
...
```{todo}
Retire each shim as its ticket ships: the `--python-flag=-m` computation on [Nuitka#3879](https://github.com/Nuitka/Nuitka/issues/3879), the symlink-free staging copy on [Nuitka#3994](https://github.com/Nuitka/Nuitka/issues/3994) (Nuitka 4.3 at the earliest), and the `ccache.conf` `base_dir` write on [Nuitka#3996](https://github.com/Nuitka/Nuitka/issues/3996). The `sloppiness` and `CCACHE_SLOPPINESS` settings need no edit on [Nuitka#3998](https://github.com/Nuitka/Nuitka/issues/3998): they start working on their own. Two shims have no deadline at all. [Nuitka#3909](https://github.com/Nuitka/Nuitka/issues/3909) shipped in `4.2` and retired nothing, because the fix reaches `--project` builds only. On [Nuitka#3997](https://github.com/Nuitka/Nuitka/issues/3997) the maintainer has not declined the request: he asked how digests could stay current across architectures without breaking older releases whose downloads move, and said tooling for it would be welcome, so the unconditional macOS `--disable-ccache` stays until someone writes that.
```
