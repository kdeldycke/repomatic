...
- **Binary download URLs** in `docs/install.md`: version-pinned, ratcheted forward to the new release by `release-prep`'s `freeze_install_download_urls`. The `Specific version` install tab example is a separate, manually-bumped pin: update it by hand to the last released version during the docs reconciliation pass.
