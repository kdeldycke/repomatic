...
- **Module reference pages**: `[tool.repomatic] docs.apidoc-extra-args` passes `--separate`, so every module gets its own `docs/{module}.md` page. `repomatic update-docs` writes a missing page but never rewrites an existing one, so a new module also needs a hand-added entry in the toctree of its package page (`docs/repomatic.md` for a top-level module, `docs/repomatic.{subpackage}.md` otherwise). `tests/test_readme.py` fails on a module that has no page. The file inventories that `tests/test_metadata.py` checks come from `git ls-files`, so a new tracked file needs no fixture edit, while an untracked file in the checkout fails them.
