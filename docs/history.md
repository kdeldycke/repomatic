...
Sampled weekly by [`repomatic sample-metrics`](https://github.com/kdeldycke/repomatic/blob/main/repomatic/metrics.py) into [`metrics.csv`](assets/metrics.csv), after GitHub restricted its stargazer endpoints to a repository's own admins in June 2026 and left every third-party chart rendering an error card; it reopened an anonymous star history in September. This curve is reconstructed from the stars the repository *still* holds, dated to the week each was given, so it runs back to 2021 rather than starting where sampling did, and it dips wherever a follower was lost.
